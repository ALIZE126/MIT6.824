...
func (rf *Raft) AppendEntries(args *AppendEntries_Args, reply *AppendEntries_Reply) {
	...
	if args.LeaderId == rf.me {
		...
		if rf.currentTerm <= args.Term {
			...
			//rand.Seed(rf.heartbeat.UnixNano())
			//rf.random_time = rand.Int()%300 + 500
		} else {
			reply.Curr_Term = rf.currentTerm
			return
		}
...
func (rf *Raft) Convert_Leader() {
	...
	go rf.Send_heartbeat(args_AppE, reply_AppE)
}
func (rf *Raft) kickoff() {
	...
	rf.mu.Unlock()
	//electionTimeout := rf.ElectionInterval + rand.Intn(200)
	...
	for {
		if rf.vote_num >= rf.majority && rf.state == Candidate { //become a Leader
			rf.Convert_Leader()
			break
...
func (rf *Raft) Send_heartbeat(args_AppE *AppendEntries_Args, reply_AppE *AppendEntries_Reply) {
	for {
		if rf.state == Leader {
			rf.mu.Lock()
			args_AppE.LeaderId = rf.me
			args_AppE.Term = rf.currentTerm
			rf.mu.Unlock()
			for peer := 0; peer < len(rf.peers); peer++ {
				if peer != rf.me {
					go func(peer int) {
			...
			rf.mu.Lock()
			if reply_AppE.Curr_Term > rf.currentTerm {
				rf.Convert_follower(reply_AppE.Curr_Term)
				rf.mu.Unlock()
				break
			}
			rf.mu.Unlock()
...
func Make(peers []*labrpc.ClientEnd, me int,
	...
	//rf.heartbeat = time.Now()
	//rand.Seed(rf.heartbeat.UnixNano())
	//rf.random_time = rand.Int()%300 + 200
