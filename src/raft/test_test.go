...
func TestReElection2A(t *testing.T) {
	...
	DPrintf("TESTAC: %d has Disconnected", leader1)
	cfg.disconnect(leader1)
	cfg.checkOneLeader()
	DPrintf("TESTAC: check there is a leader")

	// if the old leader rejoins, that shouldn't
	// disturb the new leader. and the old leader
	// should switch to follower.
	DPrintf("TESTAC: %d rejoin", leader1)
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()
	DPrintf("TESTAC: found %d as Leader", leader2)
	...
	cfg.checkNoLeader()
	DPrintf("TESTAC:  No Leader should be elected")
	// if a quorum arises, it should elect a leader.
	cfg.connect((leader2 + 1) % servers)
	DPrintf("TESTAC: recover election")
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
	DPrintf("TESTAC: %d rejoin", leader2)
	cfg.checkOneLeader()
...
func TestFailAgree2B(t *testing.T) {
	...
	//log.Println("TEST_INFO:peer:", (leader+1)%servers, "re-connect the follower will backup")
...
func TestRejoin2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	cfg.disconnect(leader1)
	//log.Println("Disconnect leader:", leader1)
	...
	leader2 := cfg.checkOneLeader()
	cfg.disconnect(leader2)
	//log.Println("Disconnect New_leader:", leader2)
	// old leader connected again
	cfg.connect(leader1)
	//log.Println("Reconnect old_leader:", leader1)
	cfg.one(104, 2, true)
